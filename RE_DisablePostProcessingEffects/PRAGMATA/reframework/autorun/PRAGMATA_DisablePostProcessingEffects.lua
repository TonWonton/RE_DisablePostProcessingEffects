--SCRIPT INFO
local s_GUID = "PRAGMATA_DisablePostProcessingEffects"
...
local te_ToneMappingTemporalAAAlgorithm =
{
	Legacy = 0,
	Standard = 1,
	StandardV2 = 2,
...
local tbl_TemporalAAAlgorithmToToneMappingTemporalAAAlgorithm =
{
	[te_TemporalAAAlgorithm.Legacy] = te_ToneMappingTemporalAAAlgorithm.Legacy,
	[te_TemporalAAAlgorithm.Standard] = te_ToneMappingTemporalAAAlgorithm.Standard,
	[te_TemporalAAAlgorithm.StandardV2] = te_ToneMappingTemporalAAAlgorithm.StandardV2,
	[te_TemporalAAAlgorithm.StandardV3] = te_ToneMappingTemporalAAAlgorithm.StandardV3,
	[te_TemporalAAAlgorithm.Sharp] = te_ToneMappingTemporalAAAlgorithm.Sharp,
	[te_TemporalAAAlgorithm.Default] = te_ToneMappingTemporalAAAlgorithm.StandardV2
}

local tbl_TemporalAAAlgorithmNames = GenerateEnumNames(te_TemporalAAAlgorithm)
...
local tbl_vignetteToToneMappingVignette =
{
	...
	[te_Vignette.Default] = te_ToneMappingVignette.Disable,
...
local function ApplyNonPersistentSettings()
	...
	local iTemporalAAAlgorithm = tblConfig.i_taaAlgorithm
	if iTemporalAAAlgorithm ~= te_TemporalAAAlgorithm.Default then
		local iToneMappingTemporalAAAlgorithm = tbl_TemporalAAAlgorithmToToneMappingTemporalAAAlgorithm[iTemporalAAAlgorithm]
		if iToneMappingTemporalAAAlgorithm ~= nil then
			cToneMapping:call("setTemporalAAAlgorithm", iToneMappingTemporalAAAlgorithm)
		end
	end
...
local function Initialize()
	...
	local cCameraGameObject = cCameraSystem:call("getCameraObject", 0)
	if cCameraGameObject == nil then return end

	if c_toneMapping == nil then c_toneMapping = get_component(cCameraGameObject, "via.render.ToneMapping") end
	if c_toneMapping == nil then return end

	if c_ldrPostProcess == nil then c_ldrPostProcess = get_component(cCameraGameObject, "via.render.LDRPostProcess") end
	...
	if c_volumetricFogControl == nil then c_volumetricFogControl = get_component(cCameraGameObject, "via.render.VolumetricFogControl") end
	if c_volumetricFogControl == nil then return end

	local tdEnvironmentSceneManager = sdk.find_type_definition("app.EnvironmentSceneManager")
	if tdEnvironmentSceneManager == nil then return end

	local mRequestLoad = tdEnvironmentSceneManager:get_method("requestLoad")
	if mRequestLoad == nil then return end

	sdk.hook(mRequestLoad, PreOnChangeCurrentStage, PostOnChangeCurrentStage)
...
re.on_draw_ui(function()
	if imgui.tree_node(s_GUIDAndVVersion) then
		...
		bChanged, tblConfig.i_taaAlgorithm = combo("TAA algorithm", tblConfig.i_taaAlgorithm, tbl_TemporalAAAlgorithmNames)
